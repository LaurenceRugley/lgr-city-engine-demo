...
void main() {
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    `,document.head.appendChild(a),Y=document.createElement(`button`),Y.className=`hoard-bagbtn`,Y.textContent=`🎒`,Y.title=`Inventory (I)`,document.body.appendChild(Y),Y.addEventListener(`click`,()=>Ge()),X=document.createElement(`div`),X.className=`hoard-bag`,document.body.appendChild(X)}let Le=0;function Re(e,t=1.8){Pe&&(Pe.textContent=e,Pe.style.display=`block`),Le=t}let ze=Math.PI/4;function Be(e){ze=e}function Q(e,t){W={x:e,z:t}}function Ve(e){_e=e}function He(){if(typeof window<`u`&&(window.__hoardBag=Object.fromEntries(I.map(e=>[e.id,e.n]))),!X)return;let e=`<div class="panel"><h3>INVENTORY</h3><div class="slots">`;for(let t=0;t<8;t++){let n=I[t];n?e+=`<button class="slot" data-id="${n.id}" title="${ln[n.id].name}">${ln[n.id].glyph}<span class="n">${n.n}</span></button>`:e+=`<div class="slot empty"></div>`}e+=`</div><div class="craft">`,un.forEach((t,n)=>{let r=Object.keys(t.need).every(e=>ie(e)>=t.need[e]),i=Object.entries(t.need).map(([e,t])=>`${t}${ln[e].glyph}`).join(`+`);e+=`<button data-rec="${n}" ${r?``:`disabled`}>Craft ${ln[t.out].name} (${i})</button>`}),e+=`</div><button class="close">CLOSE (I)</button></div>`,X.innerHTML=e,X.querySelectorAll(`.slot[data-id]`).forEach(e=>e.addEventListener(`click`,()=>{ln[e.dataset.id].consumable&&oe(e.dataset.id)})),X.querySelectorAll(`[data-rec]`).forEach(e=>e.addEventListener(`click`,()=>se(un[+e.dataset.rec]))),X.querySelector(`.close`).addEventListener(`click`,()=>We())}function Ue(){!ce||V||(Ie=!0,X&&X.classList.add(`open`),He())}function We(){Ie=!1,X&&X.classList.remove(`open`)}function Ge(){Ie?We():Ue()}function Ke(e){H=e,he=5+e*2,pe=`spawning`}function qe(){T.x=0,T.z=0,T.vx=0,T.vz=0,T.hp=100,T.stamina=100,T.hunger=100,T.thirst=100,T.facing=0,T.iframe=0;for(let e of D)e.alive=!1;le=0,ue=0,de=0,V=!1,_e=!1,W=null;for(let e of k)e.hp=150,e.alive=!0,e.mesh.scale.set(1,1,1);for(let e of ne)e.active=!1,e.mesh.visible=!1;I.length=0;let e=[`food`,`water`,`food`,`water`,`bandage`,`scrap`];for(let t=0;t<6;t++){let n=Math.random()*Math.PI*2,r=1.6+Math.random()*(s-2);P(Math.cos(n)*r,Math.sin(n)*r,e[t])}q&&(q.style.display=`none`),m.position.set(0,i,0),m.visible=!0,Ke(1),He()}function Je(){qe()}function Ye(e){ce=e,n.visible=e;let t=e&&c;Ee&&(Ee.style.display=t?`block`:`none`),J&&(J.style.display=t?`block`:`none`),Fe&&(Fe.style.display=t?`block`:`none`),Oe&&(Oe.style.display=e?`block`:`none`),Pe&&!e&&(Pe.style.display=`none`),q&&!e&&(q.style.display=`none`),Y&&(Y.style.display=e?`block`:`none`),We();for(let e in G)G[e]=!1;K.x=K.y=0,_e=!1,e&&qe()}let Xe=new v;function Ze(e,t,n){if(!ce||Ie)return;let r=n?n.windowGlow:0;if(ge=Math.max(0,ge-e),U=Math.max(0,U-e),w.material.opacity=Math.max(0,w.material.opacity-e*8),S.material.opacity=Math.max(0,S.material.opacity-e*6),!V){de+=e,T.iframe=Math.max(0,T.iframe-e);let n=(G.d||G.arrowright?1:0)-(G.a||G.arrowleft?1:0)+K.x,r=(G.w||G.arrowup?1:0)-(G.s||G.arrowdown?1:0)+K.y,a=Math.hypot(n,r);a>1&&(n/=a,r/=a);let o=a>.05,c=(G.shift||K.y>.95)&&T.stamina>2&&o,l=Math.cos(ze),u=Math.sin(ze),h=l*n+-u*r,g=-u*n+-l*r,_=c?5.2:3,v=1-Math.exp(-14*e);T.vx+=(h*_-T.vx)*v,T.vz+=(g*_-T.vz)*v,T.x+=T.vx*e,T.z+=T.vz*e;let y=Math.hypot(T.x,T.z);y>s&&(T.x*=s/y,T.z*=s/y,T.vx=0,T.vz=0),o&&(T.facing=Math.atan2(h,g)),T.stamina=p.clamp(T.stamina+(c?-34:22)*e,0,100),T.hunger=Math.max(0,T.hunger-.9*e),T.thirst=Math.max(0,T.thirst-1.15*e),T.hunger<=0||T.thirst<=0?T.hp=Math.max(0,T.hp-3.5*e):T.hunger>55&&T.thirst>55&&T.hp<100&&(T.hp=Math.min(100,T.hp+2*e));for(let e of ne)e.active&&(e.x-T.x)**2+(e.z-T.z)**2<.3&&R(e.kind)&&(e.active=!1,e.mesh.visible=!1,He());for(let t of k){if(t.hp>=150)continue;let n=Math.cos(t.ang)*d,r=Math.sin(t.ang)*d;(n-T.x)**2+(r-T.z)**2<f*f&&(t.hp=Math.min(150,t.hp+30*e),t.hp>0&&(t.alive=!0))}T.hp<=0&&Qe(),m.position.set(T.x,i,T.z),m.rotation.y=T.facing,m.visible=!(T.iframe>0&&Math.floor(t*20)%2==0),_e&&Ce(),S.position.set(T.x,i+.06,T.z),S.rotation.z=-T.facing}let a=0;V||pe===`spawning`&&(he>0&&Math.random()<e*(H===1?3.5:6)&&(ye(r),he--),he<=0&&(pe=`fighting`));let o=0,c=1/0;for(let n=0;n<48;n++){let r=D[n];if(!r.alive){Xe.position.set(0,-60,0),Xe.scale.setScalar(0),Xe.updateMatrix(),E.setMatrixAt(n,Xe.matrix);continue}a++;let s=T.x-r.x,l=T.z-r.z,f=Math.hypot(s,l)||1;if(f<c&&(c=f),!V){let t=s/f*r.speed,n=l/f*r.speed,i=1-Math.exp(-6*e);if(r.vx+=(t-r.vx)*i,r.vz+=(n-r.vz)*i,f>.52){let t=r.x+r.vx*e,n=r.z+r.vz*e,i=Math.hypot(r.x,r.z),a=Math.hypot(t,n);if(!r.in&&i>=d&&a<4.9){let r=te(t,n);if(r.alive){let i=Math.atan2(n,t),a=4.9;t=Math.cos(i)*a,n=Math.sin(i)*a,r.hp-=15*e,r.hp<=0&&(r.hp=0,r.alive=!1)}}a<d-.1&&(r.in=!0),r.x=t,r.z=n}else T.iframe<=0&&(o=Math.max(o,r.dmg))}r.flash=Math.max(0,r.flash-e);let p=Math.sin(t*6+r.phase)*.04;Xe.position.set(r.x,i+.3*r.size*u+p,r.z),Xe.rotation.set(0,Math.atan2(r.vx,r.vz),Math.sin(t*3+r.phase)*.12),Xe.scale.setScalar(r.size*u),Xe.updateMatrix(),E.setMatrixAt(n,Xe.matrix),ee.set(an[r.type].color),r.flash>0?ee.lerp(sn,.7):ee.lerp(cn,.35*(1-r.hp/r.maxhp)),E.setColorAt(n,ee)}E.instanceMatrix.needsUpdate=!0,E.instanceColor&&(E.instanceColor.needsUpdate=!0);let l=0;for(let e of k){e.alive&&l++;let t=Math.max(0,e.hp/150);e.mesh.scale.y=e.alive?Math.max(.18,t):.12,e.mesh.position.y=i+.35*e.mesh.scale.y,e.mesh.material.color.copy(e.baseColor).lerp(cn,(1-t)*.55)}!V&&o>0&&(T.hp=Math.max(0,T.hp-o*(H===1?.5:1)),T.iframe=.6,T.hp<=0&&Qe()),!V&&pe===`fighting`&&a===0&&he<=0&&(pe=`complete`,me=2.2,Re(`WAVE ${H} CLEAR`,2)),!V&&pe===`complete`&&(me-=e,me<=0&&(Ke(H+1),Re(`WAVE ${H}`,1.4))),Le>0&&(Le-=e,Le<=0&&Pe&&(Pe.style.display=`none`)),ke&&(ke.style.width=`${T.hp}%`),Ae&&(Ae.style.width=`${T.stamina}%`),je&&(je.style.width=`${T.hunger}%`),Me&&(Me.style.width=`${T.thirst}%`),Ne&&(Ne.textContent=`WAVE ${H}   KILLS ${le}   SCORE ${ue}`),typeof window<`u`&&(window.__hoard={active:ce,dead:V,paused:Ie,hp:Math.round(T.hp),stamina:Math.round(T.stamina),hunger:Math.round(T.hunger),thirst:Math.round(T.thirst),zombies:a,barriers:l,pickups:ne.filter(e=>e.active).length,inv:Object.fromEntries(I.map(e=>[e.id,e.n])),wave:H,kills:le,score:ue,weapon:`gun`,znear:+c.toFixed(2),px:+T.x.toFixed(2),pz:+T.z.toFixed(2)})}function Qe(){V=!0,_e=!1,q&&(q.querySelector(`.ds`).innerHTML=`Wave reached: ${H}<br>Kills: ${le}<br>Score: ${ue}<br>Survived: ${de.toFixed(0)}s`,q.style.display=`flex`)}return{group:n,update:Ze,setActive:Ye,setAzimuth:Be,setAim:Q,setFiring:Ve,melee:we,reset:qe,restart:Je,openBag:Ue,closeBag:We,toggleBag:Ge,addItem:R,get player(){return T},get dead(){return V},get active(){return ce},get paused(){return Ie},get inv(){return I.map(e=>({...e}))},get nearestPickup(){let e=null,t=1e9;for(let n of ne){if(!n.active)continue;let r=(n.x-T.x)**2+(n.z-T.z)**2;r<t&&(t=r,e=n)}return e?{x:e.x,z:e.z}:null},setTarget(){}}}var{Timer:fn}=oe,pn=new URLSearchParams(window.location.search),mn=pn.get(`demo`)===`1`||pn.has(`capture`);window.__demo=mn;var hn=(pn.get(`city`)?Number(pn.get(`city`)):0)||Math.random()*1e9|0,gn=0,_n=Qt({demo:mn,citySeed:hn,profileIndex:gn}),{renderer:vn,scene:yn,rig:bn,sunRig:xn,city:Sn,landmarkFactory:Cn}=_n,wn=dn({extent:Sn.extent,plinthTop:.3});yn.add(wn.group),window.__hoardApi=wn;var Tn=!0;window.__shadows=Tn,window.__scene=`hoard`;var En=new L,Dn=new Set,On=6.5;function kn(e){Sn.group.traverse(t=>{!t.isMesh||t.userData.ground||!t.visible||(t.getWorldPosition(En),Math.hypot(En.x,En.z)<e&&(t.visible=!1,Dn.add(t)))})}function An(){for(let e of Dn)e.visible=!0;Dn.clear()}var jn=new ae,Mn=new L,Nn=new L,Pn=new Set;function Fn(){for(let e of Pn)e.material&&(e.material.opacity=1,e.material.transparent=!!e.userData._wasT,e.material.depthWrite=e.userData._wasDW!==!1);Pn.clear()}function In(e){Fn();let t=bn.camera;for(let[n,r]of[[0,0],[.7,.4],[-.7,.4]]){Nn.set(e.x+n,.6,e.z+r),Mn.subVectors(Nn,t.position);let i=Mn.length();jn.set(t.position,Mn.normalize()),jn.far=i-.4;for(let e of jn.intersectObject(Sn.group,!0)){let t=e.object;!t.material||t.userData.ground||Pn.has(t)||(t.userData._wasT=t.material.transparent,t.userData._wasDW=t.material.depthWrite,t.material.transparent=!0,t.material.opacity=.16,t.material.depthWrite=!1,Pn.add(t))}}}function Ln(){wn.setActive(!0),kn(On),bn.setMode(U.DIMETRIC),bn.setZoom(2.8,!0),bn.setTarget(wn.player.x,.6,wn.player.z,!0)}var Rn=new ae,zn=new V,Bn=new c(new L(0,1,0),-.32),Vn=new L,Hn=window.matchMedia&&window.matchMedia(`(pointer: coarse)`).matches,Un=!1,Wn=0,Gn=0,Kn=.005,qn=(e,t)=>{zn.x=e/window.innerWidth*2-1,zn.y=-(t/window.innerHeight)*2+1};vn.domElement.addEventListener(`contextmenu`,e=>e.preventDefault()),vn.domElement.addEventListener(`mousedown`,e=>{e.button===0&&(qn(e.clientX,e.clientY),wn.setFiring(!0)),e.button===2&&(Un=!0,Wn=e.clientX,Gn=e.clientY)}),window.addEventListener(`mousemove`,e=>{qn(e.clientX,e.clientY),Un&&(bn.orbit(-(e.clientX-Wn)*Kn,-(e.clientY-Gn)*Kn),Wn=e.clientX,Gn=e.clientY)}),window.addEventListener(`mouseup`,()=>{wn.setFiring(!1),Un=!1}),vn.domElement.addEventListener(`wheel`,e=>{e.preventDefault(),bn.zoomBy(Math.exp(e.deltaY*.0015))},{passive:!1}),window.addEventListener(`keydown`,e=>{(e.key===`h`||e.key===`H`)&&(Tn=!Tn,window.__shadows=Tn)}),Cn.whenReady.then(()=>{Sn.generate(hn,gn),_n.fitShadowFrustum(),An(),kn(On),window.__cityReady=!0});var Jn=new fn;Jn.connect(document);function Yn(){Jn.update();let e=Math.min(Jn.getDelta(),.1);wn.setAzimuth(bn.azimuth),Hn||(Rn.setFromCamera(zn,bn.camera),Rn.ray.intersectPlane(Bn,Vn)&&wn.setAim(Vn.x,Vn.z)),wn.update(e,Jn.getElapsed(),xn),bn.setTarget(wn.player.x,.6,wn.player.z),bn.update(e),In(wn.player),_n.updateWorld(e,Jn.getElapsed(),{shadowsOn:Tn,seasonTarget:0});let t=_n.decideStyle();_n.renderCityPipeline(t,null),requestAnimationFrame(Yn)}Ln(),Yn(),nn({key:`hoard`,title:`The Hoard`,tips:[`Move: WASD / arrows · on touch: the left thumb-stick`,`Aim: mouse / drag · Fire: hold click / the FIRE button · Melee: the MELEE button`,`Survive the waves · B: bag & crafting · Esc: exit`]}),window.addEventListener(`resize`,()=>{_n.resize()});