...
export function createQualityGovernor({ profiler, apply, targetFps = 30, strongFps = 58 } = {}) {
  ...
  let level = 0, overFrames = 0, underFrames = 0, reason = 'full';
  ...
  function update() {
    ...
    if (p95 > DOWN_MS) {
      ...
      if (overFrames >= N_DOWN && level < LADDER.length - 1) { level++; overFrames = 0; reason = `p95 ${p95.toFixed(1)}ms > ${DOWN_MS.toFixed(0)}ms`; apply(level, LADDER[level]); publish(p95); }
    } else if (p95 < UP_MS) {
      underFrames++; overFrames = 0;
      if (underFrames >= N_UP && level > 0) { level--; underFrames = 0; reason = `p95 ${p95.toFixed(1)}ms < ${UP_MS.toFixed(0)}ms (headroom)`; apply(level, LADDER[level]); publish(p95); }
  ...
  return {
    ...
    reset() { level = 0; overFrames = underFrames = 0; reason = 'full'; apply(0, LADDER[0]); publish(0); },
  };
}
