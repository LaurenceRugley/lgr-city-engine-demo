...
`;function Li({controls:e,state:t,show:n,coarse:r}){if(typeof document>`u`)return;let i=document.createElement(`style`);i.textContent=Ii,document.head.appendChild(i);let a=document.createElement(`div`);a.className=`vui`;let o=document.createElement(`div`);o.className=`vui-info`,o.innerHTML=`KEYS · WASD move · 4/5/6 cam · 0 vector · 7/8 pixel/toon · B era · T time · [ ] scrub · 9 auto · Shift+W weather · K season · G shuffle · C city · I inspect · click a building / O office · J office skin · U painted props · Esc exit · M hide UI · H shadows · P theme · drag water to ripple`;let s=(e,t,n)=>{let r=document.createElement(`button`);return r.textContent=e,n&&(r.title=n),r.addEventListener(`click`,e=>{navigator.vibrate?.(10),t(e)}),r},c=e=>{let t=document.createElement(`div`);return t.className=`seg`,{seg:t,btns:e.map(([n,r])=>{let i=s(n,()=>e.find(e=>e[1]===r)[2](),n);return i.dataset.val=r,t.appendChild(i),i})}},l=e=>{let t=document.createElement(`span`);return t.className=`clbl`,t.textContent=e,t},u=(e,t,n,r)=>{let i=document.createElement(`input`);return i.type=`range`,i.min=String(e),i.max=String(t),i.step=String(n),i.addEventListener(`input`,()=>r(parseFloat(i.value))),i},d=s(`City`,()=>e.city(),`Next city profile (C)`),f=s(`Shuffle`,()=>e.shuffle(),`New random seed (G)`),p={clear:`Clear`,rain:`Rain`,snow:`Snow`,fog:`Fog`},m=[`Spring`,`Summer`,`Autumn`,`Winter`],h=s(`Clear`,()=>e.weather(),`Cycle weather: clear→rain→snow→fog (Shift+W)`),g=s(`Spring`,()=>e.season(),`Cycle season: spring→summer→autumn→winter (K)`),_=s(`Office`,()=>e.office(),`Dive into / exit the office (O · Esc)`),v={"3d":`3D`,dressed2:`Dressed`,night2:`Night`,modern:`Modern`,charm:`Charm`},y=s(`Skin`,()=>e.officeSkin(),`Office look: 3D → dressed → night → modern → charm diffusion (J)`),b={painted:`Painted`,"3d":`Live 3D`},x=s(`Props`,()=>e.officeProps(),`Office props: painted (cohesive) ↔ live 3D (animated) — under a skin (U)`),S=s(`Inspect`,()=>e.inspect(),`Inspect: fly + click/tap any car·person·bird·boat·cloud to follow + watch its behaviour (I)`),C={valley:`Valley`,archipelago:`Archi`,mountains:`Mountains`,plains:`Plains`},w=s(`World`,()=>e.world(),`Generate + explore a procedural terrain world`),T=s(`🎲`,()=>e.worldReroll(),`New random world (seed) — G`),E=s(`Valley`,()=>e.worldPreset(),`Cycle biome preset: valley → archipelago → mountains → plains`),D=s(`✎ Edit`,()=>e.sculpt&&e.sculpt(),`World editor — brush the terrain (Sculpt to reshape, Paint to recolour)`),O=document.createElement(`div`);O.className=`vui-rail`;let k=null;function ee(t){k||!t||(k=t.map(t=>{let n=document.createElement(`button`);return n.dataset.id=t.id,n.title=`${t.label} (${t.key})`,n.innerHTML=`${t.icon}<span class="rk">${t.key}</span>`,n.addEventListener(`click`,()=>{navigator.vibrate?.(10),e.editTool&&e.editTool(t.id)}),O.appendChild(n),n}))}let A=document.createElement(`div`);A.className=`vui-card`;let te=document.createElement(`div`);te.className=`ct`;let j=document.createElement(`div`);j.className=`crow`;let M=u(.8,6,.1,t=>e.brushSize&&e.brushSize(t));j.append(l(`Size`),M);let N=document.createElement(`div`);N.className=`crow`;let ne=u(.01,.15,.005,t=>e.brushStrength&&e.brushStrength(t));N.append(l(`Force`),ne);let P=document.createElement(`div`);P.className=`crow`;let F=u(.1,1,.05,t=>e.brushDensity&&e.brushDensity(t));P.append(l(`Density`),F);let re=s(`↑ Raise`,()=>e.sculptDir&&e.sculptDir(),`Brush direction: raise ↔ lower / add ↔ erase / place ↔ delete`),I=s(`↶ Undo`,()=>e.worldUndo&&e.worldUndo(),`Undo the last edit (Z)`),ie=s(`↺ Reset`,()=>e.worldReset&&e.worldReset(),`Reset to the generated world (discard edits) — same seed, NOT a reroll`),L=s(`↺ Reset`,()=>e.worldReset&&e.worldReset(),`Reset the world (discard edits) — same seed`),ae=s(`👁 Trees`,()=>e.hideScatter&&e.hideScatter(),`Hide the scatter (trees/rocks) to see the ground you are editing`),oe=document.createElement(`div`);oe.className=`seg`,oe.style.display=`none`;let se=null;function ce(t){se||!t||(se=t.map((t,n)=>{let r=document.createElement(`button`);return r.title=t.key,r.style.cssText=`min-width:30px;padding:0;background:${t.color};border:0;border-radius:8px;`,r.addEventListener(`click`,()=>{navigator.vibrate?.(8),e.material&&e.material(n)}),oe.appendChild(r),r}))}let R=document.createElement(`div`);R.className=`seg`,R.style.display=`none`;let le=null;function ue(t){le||!t||(le=t.map(t=>{let n=document.createElement(`button`);return n.dataset.key=t.key,n.textContent=t.icon,n.title=t.label,n.style.cssText=`min-width:30px;padding:4px 6px;`,n.addEventListener(`click`,()=>{navigator.vibrate?.(8),e.scatterType&&e.scatterType(t.key)}),R.appendChild(n),n}))}let z=document.createElement(`div`);z.className=`seg`,z.style.display=`none`;let B=null;function de(t){B||!t||(B=t.map(t=>{let n=document.createElement(`button`);return n.dataset.key=t.key,n.textContent=t.icon,n.title=t.label,n.style.cssText=`min-width:30px;padding:4px 6px;`,n.addEventListener(`click`,()=>{navigator.vibrate?.(8),e.entity&&e.entity(t.key)}),z.appendChild(n),n}))}let V=c([[`×1`,`1`,()=>e.dropN&&e.dropN(1)],[`×10`,`10`,()=>e.dropN&&e.dropN(10)],[`×50`,`50`,()=>e.dropN&&e.dropN(50)]]);V.seg.style.display=`none`,V.seg.title=`How many to drop per click (scattered in the ring)`;let H=document.createElement(`div`);H.className=`crow`,H.append(re,I,L,ae),A.append(te,j,N,oe,R,P,z,V.seg,H);let fe=document.createElement(`div`);fe.className=`vui-save`;let U=document.createElement(`div`);U.className=`ct`,U.textContent=`💾 Save / Load`;let W=document.createElement(`input`);W.type=`text`,W.placeholder=`world name`,W.value=`my-world`;let pe=document.createElement(`select`),me=``;pe.addEventListener(`change`,()=>{pe.value&&(W.value=pe.value)});let he=document.createElement(`input`);he.type=`file`,he.accept=`.json,application/json`,he.style.display=`none`,he.addEventListener(`change`,()=>{he.files[0]&&(e.importWorld(he.files[0]),he.value=``)});let G=s(`💾 Save`,()=>e.saveWorld(W.value.trim()),`Save to this device (a named slot)`),ge=s(`📂 Load`,()=>e.loadWorld(W.value.trim()),`Load the named slot`),_e=s(`🗑`,()=>e.deleteWorld(W.value.trim()),`Delete the named slot`),ve=s(`⬇ Export`,()=>e.exportWorld(W.value.trim()),`Download the world as a JSON file (portable, lossless)`),ye=s(`⬆ Import`,()=>he.click(),`Load a world from a JSON file`),be=s(`🔗 Link`,()=>e.shareLink(),`Copy a shareable link (light edits only — else use Export)`),xe=document.createElement(`div`);xe.className=`st`;let Se=document.createElement(`div`);Se.className=`srow`,Se.append(W);let Ce=document.createElement(`div`);Ce.className=`srow`,Ce.append(pe);let we=document.createElement(`div`);we.className=`srow`,we.append(G,ge,_e);let K=document.createElement(`div`);K.className=`srow`,K.append(ve,ye,be),fe.append(U,Se,Ce,we,K,xe,he);let q=s(`✨ Realistic`,()=>e.realistic(),`Cinematic beauty look — atmospheric sky, glowing sun, colour-graded (showcase)`),Te=c([[`Auto`,`auto`,()=>e.post(`auto`)],[`Pixel`,`pixel`,()=>e.post(`pixel`)],[`Toon`,`toon`,()=>e.post(`toon`)],[`None`,`none`,()=>e.post(`none`)]]);Te.btns[0].title=`AUTO: zoom morphs the style (toon → 16-bit → 8-bit → Game Boy)`,Te.btns[3].title=`NONE: raw beauty render, no post-crunch (clean flat-vector when Vector is on)`;let J=s(`Vector`,()=>e.vector(),`Flat-vector materials — LAYERS onto the post mode (Vector + Pixel/Toon/Auto). Toggle (0)`),Ee={native:`Era`,gb:`GB`,"8-bit":`8-bit`,"16-bit":`16-bit`,modern:`Modern`,"1-bit":`1-bit`},De=s(`Era`,()=>e.era(),`Cycle the pixel era (B): native → GB → 8-bit → 16-bit → Modern`),Y=document.createElement(`input`);Y.type=`range`,Y.min=`0`,Y.max=`1`,Y.step=`0.01`,Y.title=`Time of day`;let Oe=!1;Y.addEventListener(`pointerdown`,()=>{Oe=!0}),Y.addEventListener(`pointerup`,()=>{Oe=!1}),Y.addEventListener(`input`,()=>e.time(parseFloat(Y.value)));let ke=s(`▶`,()=>e.auto(),`Play/pause day cycle (9)`),Ae=document.createElement(`div`);Ae.style.cssText=`display:flex;align-items:center;gap:6px;`;let je=document.createElement(`span`);je.className=`lbl`,je.textContent=`Day`,Ae.append(je,Y,ke);let Me=c([[`Iso`,`iso`,()=>e.cam(`iso`)],[`Dim`,`dimetric`,()=>e.cam(`dimetric`)],[`3D`,`persp`,()=>e.cam(`persp`)]]),Ne=s(`ⓘ`,()=>o.classList.toggle(`open`),`All keys`),Pe=s(`⌄`,()=>Xe(!0),`Hide controls — watch unobstructed (M)`),Fe=document.createElement(`div`);Fe.className=`vui-more`;let Ie=s(`More`,()=>{Fe.classList.toggle(`open`),Qe()},`More controls`);if(r){a.append(d,f,_,S,w,ke,Te.seg,Ie,Pe);let e=document.createElement(`div`);e.style.cssText=`display:flex;align-items:center;gap:6px;`,e.append(je,Y),Fe.append(q,T,ie,E,D,h,g,y,x,J,De,Me.seg,e)}else a.append(d,f,at(),h,g,_,y,x,at(),S,w,T,ie,E,D,at(),Te.seg,q,J,De,at(),Ae,at(),Me.seg,Ne,at(),Pe);let Le=document.createElement(`div`);Le.className=`vui-inspect`;let Re=document.createElement(`div`);Re.className=`ik`;let ze=document.createElement(`div`);ze.className=`it`;let Be=document.createElement(`div`);Be.className=`ii`;let Ve=document.createElement(`div`);Ve.className=`ir`;let He=s(`▸ Next`,()=>e.inspectNext&&e.inspectNext(),`Follow the next object (Tab)`),Ue=s(`✕`,()=>e.inspect(),`Exit inspect (Esc)`);Ve.append(He,Ue),Le.append(Re,ze,Be,Ve);let X=document.createElement(`button`);X.className=`vui-show`,X.innerHTML=`⌃ Controls`,X.title=`Show controls (M)`,X.addEventListener(`click`,()=>Xe(!1));let We=document.createElement(`div`);We.className=`vui-style`,a.setAttribute(`role`,`toolbar`),a.setAttribute(`aria-label`,`Global controls`),O.setAttribute(`role`,`toolbar`),O.setAttribute(`aria-label`,`Tools`),O.setAttribute(`aria-orientation`,`vertical`),A.setAttribute(`role`,`group`),A.setAttribute(`aria-label`,`Tool properties`),fe.setAttribute(`role`,`group`),fe.setAttribute(`aria-label`,`Save and load worlds`);let Ge=document.createElement(`div`);Ge.className=`vui-live`,Ge.setAttribute(`role`,`status`),Ge.setAttribute(`aria-live`,`polite`),document.body.append(o,Fe,a,X,We,Le,O,A,fe,Ge);let Ke=n,Z=!1;Xe(r);let qe=null;function Je(){let e=t();Te.btns.forEach(t=>t.classList.toggle(`on`,t.dataset.val===e.post)),J.classList.toggle(`on`,!!e.vector),Me.btns.forEach(t=>t.classList.toggle(`on`,t.dataset.val===e.cam)),h.textContent=p[e.weather]||`Clear`,h.classList.toggle(`on`,e.weather!==`clear`),g.textContent=m[e.season]||`Spring`,g.classList.toggle(`on`,(e.season||0)>0),_.textContent=e.office?`Exit`:`Office`,_.classList.toggle(`on`,!!e.office),y.textContent=v[e.officeSkin]||`Skin`,y.classList.toggle(`on`,e.officeSkin&&e.officeSkin!==`3d`),x.textContent=b[e.officeProps]||`Props`,x.classList.toggle(`on`,e.officeProps===`painted`&&e.officeSkin&&e.officeSkin!==`3d`),ke.textContent=e.auto?`❚❚`:`▶`,ke.classList.toggle(`on`,e.auto),De.textContent=Ee[e.era]||`Era`,De.classList.toggle(`on`,e.era&&e.era!==`native`),S.textContent=e.inspect?`Exit`:`Inspect`,S.classList.toggle(`on`,!!e.inspect),w.textContent=e.world?`Exit`:`World`,w.classList.toggle(`on`,!!e.world),E.textContent=C[e.worldPreset]||`Valley`,T.style.display=e.world?``:`none`,ie.style.display=e.world?``:`none`,E.style.display=e.world?``:`none`,D.style.display=e.world?``:`none`;let n=e.sceneMode===`city`&&!e.worldMode,r=e.sceneMode===`city`&&e.worldMode,i=e.sceneMode===`office`,a=n||r;d.style.display=n?``:`none`,f.style.display=n?``:`none`,S.style.display=n?``:`none`,y.style.display=i?``:`none`,x.style.display=i?``:`none`,h.style.display=a?``:`none`,g.style.display=a?``:`none`,w.style.display=e.sceneMode===`city`?``:`none`;let o=r?e.sculpt?`World editor — editing`:`World`:i?`Office`:e.sceneMode===`hoard`?`Hoard`:`City`;o!==qe&&(qe=o,Ge.textContent=`${o} mode — controls updated`),e.preview&&(_.style.display=`none`),D.classList.toggle(`on`,!!e.sculpt),ee(e.tools),O.classList.toggle(`open`,!!e.sculpt),k&&k.forEach(t=>t.classList.toggle(`on`,t.dataset.id===e.editTool)),A.classList.toggle(`open`,!!e.sculpt);let s=e.editTool,c=s===`paint`,l=s===`scatter`,u=s===`place`,L=s===`sculpt`;te.textContent={place:`✚ Place`,sculpt:`⛰ Sculpt`,paint:`🎨 Paint`,scatter:`🌲 Objects`,select:`◳ Select`}[s]||`Editor`,j.style.display=s===`select`?`none`:``,N.style.display=L?``:`none`,P.style.display=l?``:`none`,ce(e.materials),oe.style.display=c?``:`none`,se&&se.forEach((t,n)=>t.classList.toggle(`on`,n===e.material)),ue(e.scatterKinds),R.style.display=l?``:`none`,le&&le.forEach(t=>t.classList.toggle(`on`,t.dataset.key===e.scatterType)),de(e.entityKinds),z.style.display=u?``:`none`,B&&B.forEach(t=>t.classList.toggle(`on`,t.dataset.key===e.entityKind)),V.seg.style.display=u?``:`none`,V.btns.forEach(t=>t.classList.toggle(`on`,t.dataset.val===String(e.dropN))),re.style.display=l||u||L?``:`none`,re.textContent=u?e.sculptRaise?`➕ Place`:`🗑 Delete`:l?e.sculptRaise?`➕ Add`:`➖ Erase`:e.sculptRaise?`↑ Raise`:`↓ Lower`,I.disabled=!e.canUndo,I.style.opacity=e.canUndo?``:`0.45`,ae.classList.toggle(`on`,!!e.scatterHidden),M.value=String(e.brushRadius),ne.value=String(e.brushStrength),F.value=String(e.brushDensity),fe.classList.toggle(`open`,!!e.sculpt);let H=e.saveSlots||[],U=H.join(`,`);if(U!==me){me=U,pe.innerHTML=``;let e=document.createElement(`option`);e.value=``,e.textContent=H.length?`— ${H.length} saved —`:`— no saves —`,pe.append(e);for(let e of H){let t=document.createElement(`option`);t.value=e,t.textContent=e,pe.append(t)}}xe.textContent=e.saveStatus||``,q.classList.toggle(`on`,!!e.realistic),Oe||(Y.value=String(e.t))}Je();let Ye=setInterval(Je,200);function Xe(e){if(!Ke){a.style.display=`none`,X.classList.remove(`on`),o.classList.remove(`open`),Fe.classList.remove(`open`),We.classList.remove(`on`);return}Z=e,a.style.display=e?`none`:`flex`,X.classList.toggle(`on`,e),e&&(o.classList.remove(`open`),Fe.classList.remove(`open`),We.classList.remove(`on`))}function Ze(){Xe(!Z)}function Qe(){if(!Fe.classList.contains(`open`))return;let e=a.getBoundingClientRect();Fe.style.top=Math.round(e.bottom+8)+`px`,Fe.style.bottom=`auto`}let $e=()=>Qe();window.addEventListener(`resize`,$e);let et=null,tt=null;function nt(e){if(!Ke||Z){We.classList.remove(`on`),et=null;return}if(!e){We.classList.remove(`on`),et=``;return}e!==et&&(et=e,We.textContent=e,We.classList.add(`on`),clearTimeout(tt),tt=setTimeout(()=>We.classList.remove(`on`),2e3))}let rt=null;function it(e){if(!Ke||!e){Le.classList.remove(`open`),rt=null;return}let t=e.hint?`hint:${e.hint}`:`${e.kind}|${e.info}`;t!==rt&&(rt=t,e.hint?(Re.textContent=`INSPECT`,ze.textContent=`Free-fly`,Be.textContent=e.hint,He.style.display=``):(Re.textContent=e.kind,ze.textContent=e.label||e.kind,Be.textContent=e.info||``,He.style.display=``),Le.classList.add(`open`))}return[a,o,Fe,X,We,Le,O,A,fe,i].forEach(e=>e&&e.querySelectorAll&&e.querySelectorAll(`button[title]:not([aria-label])`).forEach(e=>e.setAttribute(`aria-label`,e.title))),{toggle:Ze,setHidden:Xe,refresh:Je,setStyleHint:nt,setInspect:it,destroy(){clearInterval(Ye),window.removeEventListener(`resize`,$e),a.remove(),o.remove(),Fe.remove(),X.remove(),We.remove(),Le.remove(),O.remove(),A.remove(),fe.remove(),Ge.remove(),i.remove(),clearTimeout(tt)}};function at(){let e=document.createElement(`div`);return e.className=`sep`,e}}var Ri=`lgr_hints_`,zi=!1;function Bi(){if(zi||typeof document>`u`)return;zi=!0;let e=document.createElement(`style`);e.textContent=`
