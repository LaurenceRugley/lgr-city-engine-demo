...
void main() {
  ...
  `,document.head.appendChild(e);let t=document.createElement(`div`);t.className=`lap`,t.innerHTML=`<div class="lap-win"><div class="lap-bar"><b>PORTFOLIO OS — John's Game</b>
    ...
    </div></div>`;let n=document.createElement(`div`);n.className=`otoast`;let r=document.createElement(`div`);r.className=`oflash`,document.body.append(t,n,r),t.querySelector(`.lap-x`).addEventListener(`click`,()=>Hr()),t.addEventListener(`click`,e=>{e.target===t&&Hr()});let i=0;return{showLap(e){t.classList.toggle(`on`,e)},toast(e){n.textContent=e,n.classList.add(`on`),clearTimeout(i),i=setTimeout(()=>n.classList.remove(`on`),1400)},flash(){r.style.transition=`none`,r.style.opacity=`0.85`,requestAnimationFrame(()=>{r.style.transition=`opacity .55s`,r.style.opacity=`0`})}}})();function Vr(){zr=!0,Br.showLap(!0)}function Hr(){zr=!1,Br.showLap(!1)}function Ur(){Kn=(Kn+1)%Qe.length,Br.flash(),nr(),Br.toast(`✈  `+Y.state.profile.name),window.__profile=Y.state.profile.key}function Wr(e){let t=Z.setFitout(e);return Br.toast(t===`basement`?`🏚  Basement office`:`🏙  Corner office`),window.__tier=t,t}function Gr(){return Wr(Z.tier===`corner`?`basement`:`corner`)}window.__tier=Z.tier;var Kr=[`3d`,`smooth`,`charm`],qr={"3d":`🧊  Stylized 3D office`,smooth:`🖼  Smooth diffusion skin`,charm:`🕹  Charm diffusion skin`};function Jr(e){let t=Z.setSkin(e);return window.__officeSkin=t,Q!==`city`&&Br.toast(qr[t]),t}function Yr(){return Jr(Kr[(Kr.indexOf(Z.skin)+1)%Kr.length])}window.__officeSkin=Z.skin;var Xr={painted:`🎨  Painted props (cohesive)`,"3d":`🧊  Live 3D props (animated)`};function Zr(e){let t=Z.setProps(e);return window.__officeProps=t,Q!==`city`&&Z.skin!==`3d`&&Br.toast(Xr[t]),t}function Qr(){return Zr(Z.props===`painted`?`3d`:`painted`)}window.__officeProps=Z.props;function $r(e,t){Hn.visible=!1,W.setRenderTarget(In),W.render(K,e),Hn.visible=!0,W.setRenderTarget(t),W.render(K,e)}function ei(e,t){if(Hn.visible=!1,W.setRenderTarget(In),W.render(K,q.camera),Hn.visible=!0,$===1||oi&&$!==7&&$!==8)W.setRenderTarget(t),W.render(K,q.camera);else if(W.setRenderTarget(ir),W.render(K,q.camera),$===2)dr.uniforms.uGrain.value=1,dr.uniforms.uChroma.value=1,Dr(dr,t);else{dr.uniforms.uGrain.value=0,dr.uniforms.uChroma.value=0,Dr(dr,ar);let n=q.camera;Tr.uniforms.uNear.value=n.near,Tr.uniforms.uFar.value=n.far,Tr.uniforms.uIsPerspective.value=+!!n.isPerspectiveCamera;let r=e.era?(Sr(e.era),vr):wr();e.kind===`pixel`?(Dr(r,t),window.__style=`pixel`):e.kind===`toon`?(Dr(Tr,t),window.__style=`toon`):(Dr(Tr,or),Dr(r,sr),Er.uniforms.uBlend.value=e.blend,Dr(Er,t),window.__style=`blend`)}}var ti=.3,ni=.46,ri=.62,ii=.8,$=3,ai=!1,oi=!1;window.__mode=$,window.__camMode=q.mode,window.__vector=oi,window.addEventListener(`keydown`,e=>{(e.key===`1`||e.key===`2`||e.key===`3`)&&($=Number(e.key),window.__mode=$),(e.key===`7`||e.key===`8`)&&($=Number(e.key),window.__mode=$),e.key===`4`&&(q.setMode(xe.PERSPECTIVE),window.__camMode=q.mode),e.key===`5`&&(q.setMode(xe.ISOMETRIC),window.__camMode=q.mode),e.key===`6`&&(q.setMode(xe.DIMETRIC),window.__camMode=q.mode),e.key===`ArrowLeft`&&(q.pan(-1,0),e.preventDefault()),e.key===`ArrowRight`&&(q.pan(1,0),e.preventDefault()),e.key===`ArrowUp`&&(q.pan(0,1),e.preventDefault()),e.key===`ArrowDown`&&(q.pan(0,-1),e.preventDefault()),e.key===`0`&&(oi=!oi,Fe.value=+!!oi,window.__vector=oi),(e.key===`w`||e.key===`W`)&&(X.cycle(),window.__weather=X.kind),(e.key===`k`||e.key===`K`)&&(Qn=(Qn+1)%Zn.length,window.__season=Zn[Qn]),(e.key===`g`||e.key===`G`)&&(Gn=Math.random()*1e9|0,nr()),(e.key===`c`||e.key===`C`)&&(Kn=(Kn+1)%Qe.length,nr()),(e.key===`h`||e.key===`H`)&&(tr=!tr,window.__shadows=tr),(e.key===`p`||e.key===`P`)&&(ai=!ai),(e.key===`b`||e.key===`B`)&&(xr=br[(br.indexOf(xr)+1)%br.length],window.__era=xr,Cr()),(e.key===`t`||e.key===`T`)&&J.cyclePreset(),e.key===`[`&&J.nudge(-.5),e.key===`]`&&J.nudge(.5),e.key===`9`&&(J.toggleAuto(),window.__auto=J.auto),e.key===`Escape`&&(zr?Hr():Rr()),(e.key===`o`||e.key===`O`)&&(Q===`city`?Lr():Rr()),(e.key===`f`||e.key===`F`)&&Q!==`city`&&Gr(),(e.key===`j`||e.key===`J`)&&Yr(),(e.key===`u`||e.key===`U`)&&Qr(),(e.key===`m`||e.key===`M`)&&Ir&&Ir.toggle()});var si=window.matchMedia(`(prefers-reduced-motion: reduce)`);J.setReducedMotion(si.matches),si.addEventListener(`change`,e=>J.setReducedMotion(e.matches));var ci=new T,li=new z,ui=!1,di=!1,fi=0,pi=0,mi=.005,hi=(e,t)=>{li.x=e/window.innerWidth*2-1,li.y=-(t/window.innerHeight)*2+1},gi=0,_i=0,vi=0;function yi(){return ci.setFromCamera(li,q.camera),ci.intersectObject(Y.group,!0)[0]?new z(li.x*.5+.5,li.y*.5+.5):null}function bi(){ci.setFromCamera(li,Z.camera);let e=ci.intersectObjects(Z.interactables,!0)[0];if(!e)return null;let t=e.object;for(;t&&!t.userData.role;)t=t.parent;return t?t.userData.role:null}W.domElement.addEventListener(`contextmenu`,e=>e.preventDefault()),W.domElement.addEventListener(`mousedown`,e=>{e.button===0&&(ui=Q===`city`,hi(e.clientX,e.clientY),gi=e.clientX,_i=e.clientY,vi=performance.now()),e.button===2&&(di=!0,fi=e.clientX,pi=e.clientY)}),window.addEventListener(`mousemove`,e=>{ui&&hi(e.clientX,e.clientY),di?(q.orbit(-(e.clientX-fi)*mi,-(e.clientY-pi)*mi),fi=e.clientX,pi=e.clientY):Q===`city`&&!ui?(hi(e.clientX,e.clientY),W.domElement.style.cursor=yi()?`pointer`:`default`):Q===`office`&&(hi(e.clientX,e.clientY),W.domElement.style.cursor=bi()?`pointer`:`default`)}),window.addEventListener(`mouseup`,e=>{let t=Math.hypot(e.clientX-gi,e.clientY-_i)<6&&performance.now()-vi<350;if(ui&&Q===`city`&&t){hi(e.clientX,e.clientY);let t=yi();t&&Lr(t)}else if(Q===`office`&&t&&!zr){hi(e.clientX,e.clientY);let t=bi();t===`laptop`?Vr():t===`phone`?Ur():t===`cat`?Z.petCat():t===`tank`&&Z.feedFish()}ui=!1,di=!1}),W.domElement.addEventListener(`wheel`,e=>{e.preventDefault(),q.zoomBy(Math.exp(e.deltaY*.0015))},{passive:!1});var xi=0,Si=(e,t)=>[(e.clientX+t.clientX)/2,(e.clientY+t.clientY)/2],Ci=(e,t)=>Math.hypot(e.clientX-t.clientX,e.clientY-t.clientY),wi=!1;W.domElement.addEventListener(`touchstart`,e=>{e.touches.length===1&&(ui=Q===`city`,hi(e.touches[0].clientX,e.touches[0].clientY),gi=e.touches[0].clientX,_i=e.touches[0].clientY,vi=performance.now(),wi=!1),e.touches.length===2&&(ui=!1,di=!0,[fi,pi]=Si(e.touches[0],e.touches[1]),xi=Ci(e.touches[0],e.touches[1]))},{passive:!1}),W.domElement.addEventListener(`touchmove`,e=>{if(e.preventDefault(),e.touches.length===1)hi(e.touches[0].clientX,e.touches[0].clientY),Math.hypot(e.touches[0].clientX-gi,e.touches[0].clientY-_i)>8&&(wi=!0);else if(e.touches.length===2){let[t,n]=Si(e.touches[0],e.touches[1]);q.orbit(-(t-fi)*mi,-(n-pi)*mi),fi=t,pi=n;let r=Ci(e.touches[0],e.touches[1]);xi>0&&q.zoomBy(xi/r),xi=r}},{passive:!1}),window.addEventListener(`touchend`,e=>{if(!wi&&performance.now()-vi<350&&(!e.touches||e.touches.length===0)){if(Q===`city`){let e=yi();e&&Lr(e)}else if(Q===`office`&&!zr){let e=bi();e===`laptop`?Vr():e===`phone`?Ur():e===`cat`?Z.petCat():e===`tank`&&Z.feedFish()}}ui=!1,di=!1,xi=0,e.touches&&e.touches.length===1&&(ui=!0)});var Ti=new re;Ti.connect(document);var Ei=0,Di=performance.now();function Oi(){if($===1||$===2)return{kind:`none`};if($===7)return{kind:`pixel`};if($===8)return{kind:`toon`};let e=q.styleT;return window.__styleT=e,e<=ti?{kind:`toon`}:e>=ni?{kind:`pixel`,era:e<ri?`16-bit`:e<ii?`8-bit`:`gb`}:{kind:`blend`,blend:b.smoothstep(e,ti,ni),era:`16-bit`}}var ki={"16-bit":`16-bit`,"8-bit":`8-bit`,gb:`Game Boy`,modern:`Modern`,native:`Pixel`,"1-bit":`1-bit`};function Ai(e){return $===1||$===2?``:oi&&$!==7&&$!==8?`Vector`:e.kind===`toon`?`Toon`:e.kind===`pixel`?ki[e.era||xr]||`Pixel`:e.kind===`blend`?`Toon → `+(ki[e.era]||`Pixel`):``}var ji=Sn.get(`ui`)!==`0`&&!Sn.has(`capture`),Mi=!!(window.matchMedia&&window.matchMedia(`(pointer: coarse)`).matches),Ni=Cn||!ji||Mi,Pi=Ni?null:document.querySelector(`.hint`);if(Ni){let e=document.querySelector(`.hint`);e&&(e.style.display=`none`)}var Fi=Pi?Pi.textContent:``,Ii=``,Li=``;function Ri(e){!Pi||e===Ii||(Ii=e,Pi.textContent=`${Fi} · ${Li} · ${e}`)}function zi(){Li=`seed ${Y.state.seed} · ${Y.state.profile.name}`,window.__profile=Y.state.profile.key,Ii=``}zi();function Bi(){Ti.update();let e=Math.min(Ti.getDelta(),.1);q.update(e),Bn.material.uniforms.uTime.value=Ti.getElapsed(),dr.uniforms.uTime.value=Ti.getElapsed(),J.update(e),Y.key.position.copy(J.sunDir).multiplyScalar(er),Y.key.color.copy(J.sunColor),Y.key.intensity=J.sunIntensity,Y.fill.color.copy(J.hemiSky),Y.fill.groundColor.copy(J.hemiGround),Un.value=J.windowGlow;let t=X.overcast;Y.key.intensity*=1-.5*t,Y.key.color.lerp(En,.45*t),Y.fill.intensity=1+.7*t;let n=b.smoothstep(J.sunDir.y,.06,.34),r=b.lerp(.28,1,1-J.windowGlow),i=tr?n*r:0;Y.key.shadow.intensity=.72*i,Le.value=.52*i;let a=1-J.windowGlow;Ie.setRGB(b.lerp(.46,1,a),b.lerp(.52,1,a),b.lerp(.74,1,a)),dr.uniforms.uExposure.value=J.exposure,Tr.uniforms.uToonGain.value=J.toonGain,W.setClearColor(J.horizon,1),_r(J.t),Ri(J.clock),window.__t=J.t,Jn.update(e,Ti.getElapsed(),J),Y.update(Ti.getElapsed()),Yn.update(e,Ti.getElapsed(),J),Fn.uniforms.uWakeCount.value=Yn.wakeCount,X.update(e,Ti.getElapsed()),Fn.uniforms.uRainCount.value=X.rainDropCount;let o=X.fog*(1-a);K.fog.density=X.fog*Dn,kn.copy(J.horizon).lerp(On,.85*o),K.fog.color.copy(kn),W.setClearColor(kn,1),Ve.value=X.fog,Bn.material.uniforms.uFogAmt.value=.7*X.fog,Re.value=X.snow,ze.value=X.cloud*.55,Be.x+=e*.018,Be.y+=e*.009,He.value+=(Zn[Qn]-He.value)*Math.min(1,e*1.5),Xn.update(e,Ti.getElapsed(),J,X);let s=Oi(),c=s.kind===`toon`?1:s.kind===`blend`?1-s.blend:0;Vn.uniforms.uChromaScale.value=b.lerp(1,.5,c),Ir&&Ir.setStyleHint(Q===`city`?Ai(s):``),Ei++;let l=performance.now();l-Di>=1e3&&(window.__fps=Ei,Ei=0,Di=l);let u=0;if(ui&&Q===`city`){ci.setFromCamera(li,q.camera);let e=ci.intersectObject(Hn)[0];e&&e.uv&&(Fn.uniforms.uMouse.value.copy(e.uv),u=.06)}Fn.uniforms.uMouseStrength.value=u;let[d,f,p]=Mn;Fn.uniforms.uPrev.value=d.texture,Fn.uniforms.uCurr.value=f.texture,W.setRenderTarget(p),W.render(Nn,Pn),Mn=[f,p,d],Vn.uniforms.uHeight.value=Mn[1].texture,Pr+=(+(Q===`office`||Q===`diving-in`)-Pr)*Math.min(1,e*Fr),Q===`diving-in`&&Pr>.992&&(Pr=1,Q=`office`,window.__scene=Q),Q===`diving-out`&&Pr<.008&&(Pr=0,Q=`city`,window.__scene=Q),Q===`city`?ei(s,null):(Z.update(e,Ti.getElapsed(),J),Z.tier===`basement`?(W.setRenderTarget(Ar),W.render(Z.vignette.scene,Z.vignette.camera)):$r(Or,kr),Q===`office`?(W.setRenderTarget(null),W.render(Z.scene,Z.camera)):(ei(s,jr),W.setRenderTarget(Mr),W.render(Z.scene,Z.camera),Nr.uniforms.uT.value=Pr,Dr(Nr,null))),requestAnimationFrame(Bi)}var Vi={at:(e,t)=>{hi(e,t),ui=!0},stop:()=>{ui=!1}};function Hi(){let e=window.__style||($===1?`raw`:$===2?`filmic`:`auto`);return{lesson:23,clock:J.clock,style:(oi?`vec-`:``)+e,profile:Y.state.profile.key,weather:X.kind,scene:Q}}Wt({renderer:W,rig:q,sunRig:J,poke:Vi,getState:Hi,office:{pet:()=>Z.petCat(),feed:()=>Z.feedFish(),laptop:()=>Vr(),closeLaptop:()=>Hr(),travel:()=>Ur(),fitout:()=>Gr()}});var Ui=e=>window.dispatchEvent(new KeyboardEvent(`keydown`,{key:e})),Wi={cam:e=>Ui({iso:`5`,dimetric:`6`,persp:`4`}[e]),style:e=>{e===`auto`?(Ui(`3`),oi&&Ui(`0`)):e===`vector`?(oi||Ui(`0`),($===7||$===8)&&Ui(`2`)):e===`pixel`?Ui(`7`):e===`toon`&&Ui(`8`)},era:()=>Ui(`b`),city:()=>Ui(`C`),shuffle:()=>Ui(`G`),weather:()=>Ui(`W`),season:()=>Ui(`K`),office:()=>Ui(`o`),officeSkin:()=>Ui(`j`),officeProps:()=>Ui(`u`),time:e=>J.goTo(e),auto:()=>Ui(`9`)};Ir=qt({controls:Wi,state:()=>({cam:{4:`persp`,5:`iso`,6:`dimetric`}[q.mode],style:$===7?`pixel`:$===8?`toon`:oi?`vector`:`auto`,era:xr,auto:J.auto,t:J.t,weather:X.kind,season:Qn,office:Q!==`city`,officeSkin:Z.skin,officeProps:Z.props}),show:Sn.get(`ui`)!==`0`&&!Sn.has(`capture`),coarse:window.matchMedia&&window.matchMedia(`(pointer: coarse)`).matches});var Gi=Sn.get(`cam`);Gi&&[`iso`,`dimetric`,`persp`].includes(Gi)&&Wi.cam(Gi);var Ki=Sn.get(`style`);[`vector`,`pixel`,`toon`].includes(Ki)&&Wi.style(Ki);var qi=Sn.get(`t`);qi!==null&&qi!==``&&!Number.isNaN(parseFloat(qi))&&J.goTo(parseFloat(qi));var Ji=Sn.get(`time`);Ji&&J.goTo({dawn:.25,noon:.5,dusk:.75,night:0}[Ji]??.5);var Yi=Sn.get(`weather`);[`clear`,`rain`,`snow`,`fog`].includes(Yi)&&(X.setKind(Yi),window.__weather=X.kind);var Xi=Sn.get(`officeskin`);[`3d`,`smooth`,`charm`].includes(Xi)&&Jr(Xi);var Zi=Sn.get(`officeprops`);[`painted`,`3d`].includes(Zi)&&Zr(Zi);var Qi=Sn.get(`office`);(Qi===`1`||Qi===`corner`||Qi===`basement`)&&(Qi===`basement`&&Wr(`basement`),Q=`office`,Pr=1,window.__scene=Q),Bi(),window.addEventListener(`resize`,()=>{q.setViewport(window.innerWidth,window.innerHeight),W.setSize(window.innerWidth,window.innerHeight);let e=W.getDrawingBufferSize(new z);In.setSize(e.x,e.y),ir.setSize(e.x,e.y),ar.setSize(e.x,e.y),or.setSize(e.x,e.y),sr.setSize(e.x,e.y),jr.setSize(e.x,e.y),Mr.setSize(e.x,e.y),Z.camera.aspect=window.innerWidth/window.innerHeight,Z.camera.updateProjectionMatrix(),Vn.uniforms.uResolution.value.set(e.x,e.y),dr.uniforms.uResolution.value.set(e.x,e.y),gr.uniforms.uResolution.value.set(e.x,e.y),vr.uniforms.uResolution.value.set(e.x,e.y),Tr.uniforms.uResolution.value.set(e.x,e.y)});