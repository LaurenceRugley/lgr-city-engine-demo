...
void main() {
  ...
  `,document.head.appendChild(e);let t=document.createElement(`div`);t.className=`lap`,t.innerHTML=`<div class="lap-win"><div class="lap-bar"><b>PORTFOLIO OS — John's Game</b>
    ...
    </div></div>`;let n=document.createElement(`div`);n.className=`otoast`;let r=document.createElement(`div`);r.className=`oflash`,document.body.append(t,n,r),t.querySelector(`.lap-x`).addEventListener(`click`,()=>Lr()),t.addEventListener(`click`,e=>{e.target===t&&Lr()});let i=0;return{showLap(e){t.classList.toggle(`on`,e)},toast(e){n.textContent=e,n.classList.add(`on`),clearTimeout(i),i=setTimeout(()=>n.classList.remove(`on`),1400)},flash(){r.style.transition=`none`,r.style.opacity=`0.85`,requestAnimationFrame(()=>{r.style.transition=`opacity .55s`,r.style.opacity=`0`})}}})();function Ir(){Pr=!0,Fr.showLap(!0)}function Lr(){Pr=!1,Fr.showLap(!1)}function Rr(){Hn=(Hn+1)%Ze.length,Fr.flash(),Qn(),Fr.toast(`✈  `+Y.state.profile.name),window.__profile=Y.state.profile.key}function zr(e){let t=Z.setFitout(e);return Fr.toast(t===`basement`?`🏚  Basement office`:`🏙  Corner office`),window.__tier=t,t}function Br(){return zr(Z.tier===`corner`?`basement`:`corner`)}window.__tier=Z.tier;function Vr(e,t){Rn.visible=!1,W.setRenderTarget(Mn),W.render(K,e),Rn.visible=!0,W.setRenderTarget(t),W.render(K,e)}function Hr(e,t){if(Rn.visible=!1,W.setRenderTarget(Mn),W.render(K,q.camera),Rn.visible=!0,$===1||Kr&&$!==7&&$!==8)W.setRenderTarget(t),W.render(K,q.camera);else if(W.setRenderTarget(er),W.render(K,q.camera),$===2)sr.uniforms.uGrain.value=1,sr.uniforms.uChroma.value=1,Sr(sr,t);else{sr.uniforms.uGrain.value=0,sr.uniforms.uChroma.value=0,Sr(sr,tr);let n=q.camera;br.uniforms.uNear.value=n.near,br.uniforms.uFar.value=n.far,br.uniforms.uIsPerspective.value=+!!n.isPerspectiveCamera,e.kind===`pixel`?(Sr(yr(),t),window.__style=`pixel`):e.kind===`toon`?(Sr(br,t),window.__style=`toon`):(Sr(br,nr),Sr(yr(),rr),xr.uniforms.uBlend.value=e.blend,Sr(xr,t),window.__style=`blend`)}}var Ur=.45,Wr=.65,$=3,Gr=!1,Kr=!1;window.__mode=$,window.__camMode=q.mode,window.__vector=Kr,window.addEventListener(`keydown`,e=>{(e.key===`1`||e.key===`2`||e.key===`3`)&&($=Number(e.key),window.__mode=$),(e.key===`7`||e.key===`8`)&&($=Number(e.key),window.__mode=$),e.key===`4`&&(q.setMode(B.PERSPECTIVE),window.__camMode=q.mode),e.key===`5`&&(q.setMode(B.ISOMETRIC),window.__camMode=q.mode),e.key===`6`&&(q.setMode(B.DIMETRIC),window.__camMode=q.mode),e.key===`ArrowLeft`&&(q.pan(-1,0),e.preventDefault()),e.key===`ArrowRight`&&(q.pan(1,0),e.preventDefault()),e.key===`ArrowUp`&&(q.pan(0,1),e.preventDefault()),e.key===`ArrowDown`&&(q.pan(0,-1),e.preventDefault()),e.key===`0`&&(Kr=!Kr,Pe.value=+!!Kr,window.__vector=Kr),(e.key===`w`||e.key===`W`)&&(X.cycle(),window.__weather=X.kind),(e.key===`k`||e.key===`K`)&&(Jn=(Jn+1)%qn.length,window.__season=qn[Jn]),(e.key===`g`||e.key===`G`)&&(Vn=Math.random()*1e9|0,Qn()),(e.key===`c`||e.key===`C`)&&(Hn=(Hn+1)%Ze.length,Qn()),(e.key===`h`||e.key===`H`)&&(Zn=!Zn,window.__shadows=Zn),(e.key===`p`||e.key===`P`)&&(Gr=!Gr),(e.key===`b`||e.key===`B`)&&(_r=gr[(gr.indexOf(_r)+1)%gr.length],window.__era=_r,vr()),(e.key===`t`||e.key===`T`)&&J.cyclePreset(),e.key===`[`&&J.nudge(-.5),e.key===`]`&&J.nudge(.5),e.key===`9`&&(J.toggleAuto(),window.__auto=J.auto),e.key===`Escape`&&(Pr?Lr():Nr()),(e.key===`o`||e.key===`O`)&&(Q===`city`?Mr():Nr()),(e.key===`f`||e.key===`F`)&&Q!==`city`&&Br(),(e.key===`m`||e.key===`M`)&&jr&&jr.toggle()});var qr=window.matchMedia(`(prefers-reduced-motion: reduce)`);J.setReducedMotion(qr.matches),qr.addEventListener(`change`,e=>J.setReducedMotion(e.matches));var Jr=new T,Yr=new N,Xr=!1,Zr=!1,Qr=0,$r=0,ei=.005,ti=(e,t)=>{Yr.x=e/window.innerWidth*2-1,Yr.y=-(t/window.innerHeight)*2+1},ni=0,ri=0,ii=0;function ai(){return Jr.setFromCamera(Yr,q.camera),Jr.intersectObject(Y.group,!0)[0]?new N(Yr.x*.5+.5,Yr.y*.5+.5):null}function oi(){Jr.setFromCamera(Yr,Z.camera);let e=Jr.intersectObjects(Z.interactables,!0)[0];if(!e)return null;let t=e.object;for(;t&&!t.userData.role;)t=t.parent;return t?t.userData.role:null}W.domElement.addEventListener(`contextmenu`,e=>e.preventDefault()),W.domElement.addEventListener(`mousedown`,e=>{e.button===0&&(Xr=Q===`city`,ti(e.clientX,e.clientY),ni=e.clientX,ri=e.clientY,ii=performance.now()),e.button===2&&(Zr=!0,Qr=e.clientX,$r=e.clientY)}),window.addEventListener(`mousemove`,e=>{Xr&&ti(e.clientX,e.clientY),Zr?(q.orbit(-(e.clientX-Qr)*ei,-(e.clientY-$r)*ei),Qr=e.clientX,$r=e.clientY):Q===`city`&&!Xr?(ti(e.clientX,e.clientY),W.domElement.style.cursor=ai()?`pointer`:`default`):Q===`office`&&(ti(e.clientX,e.clientY),W.domElement.style.cursor=oi()?`pointer`:`default`)}),window.addEventListener(`mouseup`,e=>{let t=Math.hypot(e.clientX-ni,e.clientY-ri)<6&&performance.now()-ii<350;if(Xr&&Q===`city`&&t){ti(e.clientX,e.clientY);let t=ai();t&&Mr(t)}else if(Q===`office`&&t&&!Pr){ti(e.clientX,e.clientY);let t=oi();t===`laptop`?Ir():t===`phone`?Rr():t===`cat`?Z.petCat():t===`tank`&&Z.feedFish()}Xr=!1,Zr=!1}),W.domElement.addEventListener(`wheel`,e=>{e.preventDefault(),q.zoomBy(Math.exp(e.deltaY*.0015))},{passive:!1});var si=0,ci=(e,t)=>[(e.clientX+t.clientX)/2,(e.clientY+t.clientY)/2],li=(e,t)=>Math.hypot(e.clientX-t.clientX,e.clientY-t.clientY),ui=!1;W.domElement.addEventListener(`touchstart`,e=>{e.touches.length===1&&(Xr=Q===`city`,ti(e.touches[0].clientX,e.touches[0].clientY),ni=e.touches[0].clientX,ri=e.touches[0].clientY,ii=performance.now(),ui=!1),e.touches.length===2&&(Xr=!1,Zr=!0,[Qr,$r]=ci(e.touches[0],e.touches[1]),si=li(e.touches[0],e.touches[1]))},{passive:!1}),W.domElement.addEventListener(`touchmove`,e=>{if(e.preventDefault(),e.touches.length===1)ti(e.touches[0].clientX,e.touches[0].clientY),Math.hypot(e.touches[0].clientX-ni,e.touches[0].clientY-ri)>8&&(ui=!0);else if(e.touches.length===2){let[t,n]=ci(e.touches[0],e.touches[1]);q.orbit(-(t-Qr)*ei,-(n-$r)*ei),Qr=t,$r=n;let r=li(e.touches[0],e.touches[1]);si>0&&q.zoomBy(si/r),si=r}},{passive:!1}),window.addEventListener(`touchend`,e=>{if(!ui&&performance.now()-ii<350&&(!e.touches||e.touches.length===0)){if(Q===`city`){let e=ai();e&&Mr(e)}else if(Q===`office`&&!Pr){let e=oi();e===`laptop`?Ir():e===`phone`?Rr():e===`cat`?Z.petCat():e===`tank`&&Z.feedFish()}}Xr=!1,Zr=!1,si=0,e.touches&&e.touches.length===1&&(Xr=!0)});var di=new a;di.connect(document);var fi=0,pi=performance.now();function mi(){if($===1||$===2)return{kind:`none`};if($===7)return{kind:`pixel`};if($===8)return{kind:`toon`};let e=q.styleT;return window.__styleT=e,e>=Wr?{kind:`pixel`}:e<=Ur?{kind:`toon`}:{kind:`blend`,blend:b.smoothstep(e,Ur,Wr)}}var hi=yn?null:document.querySelector(`.hint`);if(yn){let e=document.querySelector(`.hint`);e&&(e.style.display=`none`)}var gi=hi?hi.textContent:``,_i=``,vi=``;function yi(e){!hi||e===_i||(_i=e,hi.textContent=`${gi} · ${vi} · ${e}`)}function bi(){vi=`seed ${Y.state.seed} · ${Y.state.profile.name}`,window.__profile=Y.state.profile.key,_i=``}bi();function xi(){di.update();let e=Math.min(di.getDelta(),.1);q.update(e),In.material.uniforms.uTime.value=di.getElapsed(),sr.uniforms.uTime.value=di.getElapsed(),J.update(e),Y.key.position.copy(J.sunDir).multiplyScalar(Xn),Y.key.color.copy(J.sunColor),Y.key.intensity=J.sunIntensity,Y.fill.color.copy(J.hemiSky),Y.fill.groundColor.copy(J.hemiGround),zn.value=J.windowGlow;let t=X.overcast;Y.key.intensity*=1-.5*t,Y.key.color.lerp(Sn,.45*t),Y.fill.intensity=1+.7*t;let n=b.smoothstep(J.sunDir.y,.06,.34),r=b.lerp(.28,1,1-J.windowGlow),i=Zn?n*r:0;Y.key.shadow.intensity=.72*i,Ie.value=.52*i;let a=1-J.windowGlow;Fe.setRGB(b.lerp(.46,1,a),b.lerp(.52,1,a),b.lerp(.74,1,a)),sr.uniforms.uExposure.value=J.exposure,br.uniforms.uToonGain.value=J.toonGain,W.setClearColor(J.horizon,1),pr(J.t),yi(J.clock),window.__t=J.t,Wn.update(e,di.getElapsed(),J),Y.update(di.getElapsed()),Gn.update(e,di.getElapsed(),J),jn.uniforms.uWakeCount.value=Gn.wakeCount,X.update(e,di.getElapsed()),jn.uniforms.uRainCount.value=X.rainDropCount;let o=X.fog*(1-a);K.fog.density=X.fog*Cn,Tn.copy(J.horizon).lerp(wn,.85*o),K.fog.color.copy(Tn),W.setClearColor(Tn,1),Be.value=X.fog,In.material.uniforms.uFogAmt.value=.7*X.fog,Le.value=X.snow,Re.value=X.cloud*.55,ze.x+=e*.018,ze.y+=e*.009,Ve.value+=(qn[Jn]-Ve.value)*Math.min(1,e*1.5),Kn.update(e,di.getElapsed(),J,X);let s=mi(),c=s.kind===`toon`?1:s.kind===`blend`?1-s.blend:0;Ln.uniforms.uChromaScale.value=b.lerp(1,.5,c),fi++;let l=performance.now();l-pi>=1e3&&(window.__fps=fi,fi=0,pi=l);let u=0;if(Xr&&Q===`city`){Jr.setFromCamera(Yr,q.camera);let e=Jr.intersectObject(Rn)[0];e&&e.uv&&(jn.uniforms.uMouse.value.copy(e.uv),u=.06)}jn.uniforms.uMouseStrength.value=u;let[d,f,p]=On;jn.uniforms.uPrev.value=d.texture,jn.uniforms.uCurr.value=f.texture,W.setRenderTarget(p),W.render(kn,An),On=[f,p,d],Ln.uniforms.uHeight.value=On[1].texture,kr+=(+(Q===`office`||Q===`diving-in`)-kr)*Math.min(1,e*Ar),Q===`diving-in`&&kr>.992&&(kr=1,Q=`office`,window.__scene=Q),Q===`diving-out`&&kr<.008&&(kr=0,Q=`city`,window.__scene=Q),Q===`city`?Hr(s,null):(Z.update(e,di.getElapsed(),J),Z.tier===`basement`?(W.setRenderTarget(Tr),W.render(Z.vignette.scene,Z.vignette.camera)):Vr(Cr,wr),Q===`office`?(W.setRenderTarget(null),W.render(Z.scene,Z.camera)):(Hr(s,Er),W.setRenderTarget(Dr),W.render(Z.scene,Z.camera),Or.uniforms.uT.value=kr,Sr(Or,null))),requestAnimationFrame(xi)}var Si={at:(e,t)=>{ti(e,t),Xr=!0},stop:()=>{Xr=!1}};function Ci(){let e=window.__style||($===1?`raw`:$===2?`filmic`:`auto`);return{lesson:23,clock:J.clock,style:(Kr?`vec-`:``)+e,profile:Y.state.profile.key,weather:X.kind,scene:Q}}Ht({renderer:W,rig:q,sunRig:J,poke:Si,getState:Ci,office:{pet:()=>Z.petCat(),feed:()=>Z.feedFish(),laptop:()=>Ir(),closeLaptop:()=>Lr(),travel:()=>Rr(),fitout:()=>Br()}});var wi=e=>window.dispatchEvent(new KeyboardEvent(`keydown`,{key:e})),Ti={cam:e=>wi({iso:`5`,dimetric:`6`,persp:`4`}[e]),style:e=>{e===`vector`?(Kr||wi(`0`),($===7||$===8)&&wi(`2`)):e===`pixel`?wi(`7`):e===`toon`&&wi(`8`)},city:()=>wi(`C`),shuffle:()=>wi(`G`),weather:()=>wi(`W`),season:()=>wi(`K`),office:()=>wi(`o`),time:e=>J.goTo(e),auto:()=>wi(`9`)};jr=Gt({controls:Ti,state:()=>({cam:{4:`persp`,5:`iso`,6:`dimetric`}[q.mode],style:$===7?`pixel`:$===8?`toon`:`vector`,auto:J.auto,t:J.t,weather:X.kind,season:Jn,office:Q!==`city`}),show:vn.get(`ui`)!==`0`&&!vn.has(`capture`),coarse:window.matchMedia&&window.matchMedia(`(pointer: coarse)`).matches});var Ei=vn.get(`cam`);Ei&&[`iso`,`dimetric`,`persp`].includes(Ei)&&Ti.cam(Ei);var Di=vn.get(`style`);[`vector`,`pixel`,`toon`].includes(Di)&&Ti.style(Di);var Oi=vn.get(`t`);Oi!==null&&Oi!==``&&!Number.isNaN(parseFloat(Oi))&&J.goTo(parseFloat(Oi));var ki=vn.get(`time`);ki&&J.goTo({dawn:.25,noon:.5,dusk:.75,night:0}[ki]??.5);var Ai=vn.get(`weather`);[`clear`,`rain`,`snow`,`fog`].includes(Ai)&&(X.setKind(Ai),window.__weather=X.kind);var ji=vn.get(`office`);(ji===`1`||ji===`corner`||ji===`basement`)&&(ji===`basement`&&zr(`basement`),Q=`office`,kr=1,window.__scene=Q),xi(),window.addEventListener(`resize`,()=>{q.setViewport(window.innerWidth,window.innerHeight),W.setSize(window.innerWidth,window.innerHeight);let e=W.getDrawingBufferSize(new N);Mn.setSize(e.x,e.y),er.setSize(e.x,e.y),tr.setSize(e.x,e.y),nr.setSize(e.x,e.y),rr.setSize(e.x,e.y),Er.setSize(e.x,e.y),Dr.setSize(e.x,e.y),Z.camera.aspect=window.innerWidth/window.innerHeight,Z.camera.updateProjectionMatrix(),Ln.uniforms.uResolution.value.set(e.x,e.y),sr.uniforms.uResolution.value.set(e.x,e.y),fr.uniforms.uResolution.value.set(e.x,e.y),mr.uniforms.uResolution.value.set(e.x,e.y),br.uniforms.uResolution.value.set(e.x,e.y)});